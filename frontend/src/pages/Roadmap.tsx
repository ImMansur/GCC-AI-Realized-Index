...
const RoadmapPage = () => {
  ...
  const {
    scores: stateScores,
    persona: statePersona,
    role: stateRole,
    insights: stateInsights,
    answers: stateAnswers,
    roadmap: preGeneratedRoadmap,
  } = (location.state || {}) as {
    scores?: Scores;
    persona?: string;
    role?: string;
  ...
  const [scores, setScores] = useState<Scores | undefined>(stateScores);
  const [persona, setPersona] = useState<string | undefined>(statePersona);
  const [role, setRole] = useState<string | undefined>(stateRole);
  const [insights, setInsights] = useState<Record<string, string[]> | undefined>(stateInsights);
  const [answers, setAnswers] = useState<any[] | undefined>(stateAnswers);
  ...
  useEffect(() => {
    // Skip if we already have a pre-generated roadmap from navigation state
    if (preGeneratedRoadmap) return;

    // If no scores from state, try to load from the latest survey in Firebase
    if (!stateScores) {
      const loadFromFirebase = async (uid: string) => {
        try {
          const res = await fetch(`${API_BASE}/api/users/${uid}/surveys/latest`);
          if (!res.ok) throw new Error("No survey");
          ...
          if (data.survey) {
            ...
            if (data.survey.roadmap) {
              ...
              setLoading(false);
              return; // roadmap found in Firebase, no need to regenerate
            }
            // No saved roadmap — will be generated by the next effect
          }
        } catch {
          // no survey found
        }
    ...
    // We have scores from state but no roadmap — generate one
    if (!stateScores || !statePersona || !stateRole) {
      setLoading(false);
      return;
    }
      ...
      try {
        const res = await fetch(`${API_BASE}/api/roadmap/generate`, {
          ...
          body: JSON.stringify({
            persona: statePersona,
            role: stateRole,
            composite_score: stateScores.composite_score,
            dimensions: stateScores.dimensions,
  ...
  }, [stateScores, statePersona, stateRole]);
